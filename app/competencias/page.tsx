    ...
    <AppShell pageTitle="Competências">
      ...
      {!competencias || competencias.length === 0 ? (
        <Card className="flex flex-col items-center justify-center py-16 gap-3">
          ...
          <p className="text-sm text-[#52525B]">Nenhuma competência criada.</p>
        ...
        <div className="flex flex-col gap-2">
          {competencias.map((c) => (
            <Link key={c.id} href={`/competencias/${c.id}`}>
              <Card className="flex items-center justify-between hover:bg-[#151515] transition-colors cursor-pointer">
